...
#[allow(
    ...
    reason = "both-hash expiry and exhausted-retry fixture retains cumulative accounting evidence"
)]
async fn uri_download_reopens_once_with_cumulative_counters() -> TestResult {
    for format in [ObjectFormat::Sha1, ObjectFormat::Sha256] {
        for spent in [false, true] {
            let old = fixture(format, b"old")?;
            let mut new = fixture(format, &vec![b'x'; 65536])?;
            ...
            let (log, faults, _) = test_log("uri-download-expiry").await?;
            ...
            let result = repository.fetch_uri_pack(id, checksum).await;
            if spent {
                assert!(
                ...
                let reply = result?;
                assert!(reply.starts_with(b"PACK"));
                let pack = &reply;
                assert_eq!(u32::from_be_bytes(pack[8..12].try_into()?), 1);
                assert!(operation.live_bytes() >= reply.len());
                assert!(operation.work_bytes() > before_work);
                eprintln!(
                    "URI {format:?}: cumulative calls={}, work={}, retained_response_bytes={}, fetch_store_gets={}, fetch_downloaded_bytes={}",
                    operation.calls(),
